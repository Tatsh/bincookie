...
#define bincookie_domain_access_full(cookie_ptr)                                                   \
    (((char *)cookie_ptr + cookie_ptr->domain_offset)[0] == '.') /**< @hideinitializer */
 ...
 */
#define bincookie_domain(c) ((char *)c + c->domain_offset) /**< @hideinitializer */
 ...
 * \param c Pointer to a bincookie_cookie_t structure.
 */
#define bincookie_value(c) ((char *)c + c->value_offset) /**< @hideinitializer */
...
static inline bincookie_page_t *const bincookie_iter_pages(const bincookie_t *bc,
    ...
    if (state->page_offset == 0) {
        state->page_offset = (uint32_t)(4 + sizeof(uint32_t) + (sizeof(uint32_t) * bc->num_pages));
    }
    if (state->page_index < bc->num_pages) {
        bincookie_page_t *page = (bincookie_page_t *)((char *)bc + state->page_offset);
 ...
 */
static inline bincookie_cookie_t *const bincookie_iter_cookies(const bincookie_page_t *page,
                                                               unsigned int *i) {
    if (*i < page->num_cookies) {
        bincookie_cookie_t *ptr = (bincookie_cookie_t *)((char *)page + page->cookie_offsets[*i]);
        (*i)++;
        return ptr;
    }
    return NULL;
