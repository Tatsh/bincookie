...
/*!
 * \param cookie_ptr Pointer to a binarycookies_cookie_t data structure.
 */
#define binarycookies_is_secure(cookie_ptr) (cookie_ptr->flags & secure) /**< @hideinitializer */
//! Check if a cookie can be used on all subdomains of its original domain.
/*!
 * \param cookie_ptr Pointer to a binarycookies_cookie_t structure.
 */
#define binarycookies_domain_access_full(cookie_ptr) (cookie_ptr->domain && cookie_ptr->domain[0] == '.') /**< @hideinitializer */
...
/*!
 ...
 \return Pointer to a binarycookies_t structure or `NULL`.
 */
ADDAPI binarycookies_t *binarycookies_init(const char *file_path);
//! Free a binarycookies_t structure.
/*!
  \param cfile Pointer to data structure created with binarycookies_init().
 */
